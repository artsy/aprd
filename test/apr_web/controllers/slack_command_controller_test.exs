defmodule AprWeb.SlackCommandControllerTest do
  ...
  describe "subscribe" do
    ...
    test "subscribes to a topic when receiving subscribe command with proper token", %{
      topic: topic
    } do
  ...
  describe "unsubscribe" do
    ...
    test "unsubscribes from a topic when receiving unsubscribe command with proper token", %{
      subscriber: subscriber,
      topic: topic
    } do
      conn =
    ...
    test "returns proper message when receiving unsubscribe command for non-subscribed topic", %{
      subscriber: subscriber
    } do
      conn = receive_slack_message("token", "unsubscribe random", subscriber.channel_id)

      assert json_response(conn, 200) == %{
               "text" => "Can't find a matching subscription to unsubscribe!",
               "response_type" => "in_channel"
             }
